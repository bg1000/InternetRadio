...
#            print (proc.name)
            try:
                ...
                if (processName.lower() == proc.name().lower() and 
                   proc not in self.pidList):
                    self.pidList.append(proc)
            except (psutil.NoSuchProcess, psutil.AccessDenied, 
                    psutil.ZombieProcess):

                print (str(datetime.datetime.now()) + 
                        "Exception ocurred while looking for process.")
    ...
    def killAll(self):
        ...
        for p in self.pidList:
            ...
            try:
                ...
                print(str(datetime.datetime.now()) + 
                      " - Time out waiting for process to terminate. Attempting to kill")
                p.kill()
    ...
    def leaveOne(self):
        ...
        for i in range (0, self.count-1):
            ...
            try:
                ...
                print(str(datetime.datetime.now()) + 
                      "- Time out waiting for process to terminate." +  
                      "Attempting to kill.")
...
def is_running(script):
    for q in psutil.process_iter():
        if q.name().startswith('python'):
            if (len(q.cmdline())>1 and script in q.cmdline()[1] 
                   and q.pid !=os.getpid()):
